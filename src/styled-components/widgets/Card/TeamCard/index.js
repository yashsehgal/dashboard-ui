...
export default function TeamCard(__TeamCardDetails) {
  return (
    <div className="team-card">
      <div className="team-initial-banner"></div>
      <h5 className="team-name">{__TeamCardDetails.Name}</h5>
      <div className="team-member-status">
        <div className={`${checkAvailabilityStatus(__TeamCardDetails.ActiveMemberCount)} status-circle`}></div>
        <p className="active_offline-status">{__TeamCardDetails.ActiveMemberCount} / {__TeamCardDetails.MemberCount}</p>
      </div>
