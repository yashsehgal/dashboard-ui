...
export default function Chats() {
  const [
    editGroupChatDetailsModalPopupState,
    setEditGroupChatDetailsModalPopupState,
  ] = useState(false);
  const [startNewChatModalPopupState, setStartNewChatModalPopupState] =
    useState(false);
  const [currentGroupChatTitle, setCurrentGroupChatTitle] = useState(
    "GitSpace - Frontend Development Team"
  );

  const [chatChannels] = useState([
    "General",
    "News",
    "GitHub-Updates",
    "Engineering",
  ...
  return (
    <div className="view view__chats">
      <div className="chats-view-header">
        <h4 className="chats-view-title" id="group-chat-title">
          {currentGroupChatTitle}
        </h4>
        <div className="chats-view-header-button-wrapper">
          <div
            className="button-wrap-fit-content"
            onClick={() => setStartNewChatModalPopupState(true)}
          >
            <PrimaryButton value="Start a new chat" icon="fas fa-plus" />
          </div>
          <div
            className="button-wrap-fit-content"
            onClick={() => setEditGroupChatDetailsModalPopupState(true)}
          >
            <OutlineButton
              value="Edit group chat details"
              icon="fas fa-pen"
              color="danger"
            />
          </div>
        ...
        <div className="group-chat-channels-wrapper">
          <ul className="group-chat-channels-list">
            {chatChannels.map((channel, index) => (
              <li className="group-chat-channel-item" key={index}>
                #{channel.toLowerCase()}
              </li>
            ))}
          </ul>
      ...
      <ReactModal
        isOpen={editGroupChatDetailsModalPopupState}
      ...
      >
        ...
        >
          <div
            className="button-wrap-fit-content"
            onClick={() => {
              const __changedGroupChatTitle = document.getElementById(
                "changed-group-chat-title"
              );
              console.log(__changedGroupChatTitle.value);
              !__changedGroupChatTitle.value ||
              __changedGroupChatTitle.value === ""
          ...
          <div
            className="button-wrap-fit-content"
            onClick={() => setEditGroupChatDetailsModalPopupState(false)}
          >
      ...
      <ReactModal
        isOpen={startNewChatModalPopupState}
